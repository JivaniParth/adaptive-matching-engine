...
def main():
    ...
    # Convert full dataset to orders but only process a sample to avoid long runs
    ...
    # Process first N orders (or all if small)
    N = min(2000, len(orders))
    print(f"Processing {N} orders through AdaptiveMatchingEngine...")
    start = time.time()
    for o in orders[:N]:
